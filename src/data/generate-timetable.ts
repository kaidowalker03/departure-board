// このファイルはダイヤデータ生成用のスクリプトです
// 実行: npx ts-node src/data/generate-timetable.ts
...
// 5:00 ~ 翌0:30 まで1分おきに生成
function generateTimetable(): DepartureEntry[] {
  const entries: DepartureEntry[] = [];
  let order = 1;
  
  for (let totalMin = 5 * 60; totalMin <= 24 * 60 + 30; totalMin++) {
...
console.log(JSON.stringify(timetable.slice(0, 5), null, 2));
